...
ID = r'<div class="question-summary" id="question-summary-([\d]+)">'
summary = r'<h3><a href=".*?" class="question-hyperlink">(.*?)</a></h3>'
time = r'asked <span title=.*? class="relativetime">(.*?)</span>'

for line in sys.stdin:
    IDmatch = re.findall(ID, line)
    if IDmatch:
        IDs.append(IDmatch[0])

    summarymatch = re.findall(summary, line)
    if summarymatch:
        summaries.append(summarymatch[0])

    timematch = re.findall(time, line)
    if timematch:
        times.append(timematch[0])

for i in range(len(IDs)):
    print(IDs[i], summaries[i], times[i], sep=";")
